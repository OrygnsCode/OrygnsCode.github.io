    ...
    setupTabNavigation() {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                document.getElementById(`${targetTab}-tab`).classList.add('activ
e');
    ...
    setupEventListeners() {
        // Control buttons
        document.getElementById('resetBtn').addEventListener('click', () => this
.resetParameters());
        document.getElementById('optimizeBtn').addEventListener('click', () => t
his.optimizeRefluxRatio());

        // Diagram controls
        document.getElementById('zoomReset').addEventListener('click', () => thi
s.resetZoom());
        document.getElementById('toggleGrid').addEventListener('click', () => th
is.toggleGrid());
        document.getElementById('toggleAnimation').addEventListener('click', ()
=> this.toggleAnimation());

        // Export buttons
        document.getElementById('exportSVG').addEventListener('click', () => thi
s.exportSVG());
        document.getElementById('exportPNG').addEventListener('click', () => thi
s.exportPNG());
        document.getElementById('exportPDF').addEventListener('click', () => thi
s.exportPDF());
        document.getElementById('exportCSV').addEventListener('click', () => thi
s.exportCSV());
...
;
    ...
    setupVisualization() {
        ...
        this.height = containerRect.height - this.margin.top - this.margin.botto
m;
        ...
        this.zoom = d3.zoom()
            ...
            .on('zoom', (event) => {
                this.g.attr('transform',
                    `translate(${this.margin.left},${this.margin.top}) ${event.t
ransform}`);
            });
    ...
    calculateMinimumRefluxRatio() {
        const { feedComposition, distillateComposition, qValue, relativeVolatili
ty } = this.parameters;

        // Calculate y* at feed composition
        const yStarFeed = this.calculateEquilibrium(feedComposition, relativeVol
atility);
        ...
        if (qValue === 1) {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                const dydx = relativeVolatility / Math.pow(1 + (relativeVolatili
ty - 1) * x, 2);
        ...
        const minReflux = (distillateComposition - yIntersect) / (yIntersect - x
Intersect);
        return Math.max(minReflux, 0.1);
    }

    calculateOperatingLines() {
        const { feedComposition, distillateComposition, bottomsComposition, refl
uxRatio, qValue } = this.parameters;
        ...
        const xIntersect = (qLineIntercept - rectifyingIntercept) / (rectifyingS
lope - qLineSlope);
        const yIntersect = rectifyingSlope * xIntersect + rectifyingIntercept;

        // Stripping section operating line
        const strippingSlope = (yIntersect - bottomsComposition) / (xIntersect -
 bottomsComposition);
        const strippingIntercept = bottomsComposition - strippingSlope * bottoms
Composition;
    ...
    calculateTheoreticalPlates() {
        const { distillateComposition, bottomsComposition, relativeVolatility }
= this.parameters;
        ...
        while (x > bottomsComposition && plates < 100) {
            ...
            steps.push({ x1: x, y1: y, x2: x, y2: yNew, type: 'vertical', plate:
 plates + 1 });
            y = yNew;
            ...
            if (x > operatingLines.intersection.x) {
                // Rectifying section
                xNew = (y - operatingLines.rectifying.intercept) / operatingLine
s.rectifying.slope;
            } else {
                // Stripping section
                xNew = (y - operatingLines.stripping.intercept) / operatingLines
.stripping.slope;
                if (!feedPlateFound) {
            ...
            steps.push({ x1: x, y1: y, x2: xNew, y2: y, type: 'horizontal', plat
e: plates + 1 });
            ...
            plateData.push({
                ...
                x: x,
                y: y,
                section: x > operatingLines.intersection.x ? 'rectifying' : 'str
ipping'
            });
            ...
            if (x <= bottomsComposition || Math.abs(x - bottomsComposition) < 1e
-6) break;
        }
    ...
    calculateHeatDuties() {
        const { feedFlowRate, feedComposition, distillateComposition, bottomsCom
position } = this.parameters;

        // Material balance
        const distillateFlowRate = feedFlowRate * (feedComposition - bottomsComp
osition) /
        ...
        const heatOfVaporization = 35.0; // kJ/mol (approximate for light hydroc
arbons)
        const refluxFlowRate = this.parameters.refluxRatio * distillateFlowRate;

        const condenserDuty = (refluxFlowRate + distillateFlowRate) * heatOfVapo
rization / 3600; // kW
        const reboilerDuty = condenserDuty * 1.1; // Approximate, accounting for
 column heat losses
    ...
    updateCalculations() {
        try {
            ...
            const efficiency = Math.min(95, Math.max(65,
                90 - (this.parameters.refluxRatio - this.results.minRefluxRatio)
 * 1.5));
...
');
    ...
    updateVisualization() {
        ...
        this.g.selectAll('.curve, .operating-line, .plate-step, .key-point, .poi
nt-label').remove();
    ...
    drawOperatingLines() {
        const operatingLines = this.calculateOperatingLines();
        const { feedComposition, distillateComposition, bottomsComposition } = t
his.parameters;
...
his.parameters;
        ...
        const points = [
            { x: feedComposition, y: feedComposition, label: 'Feed', color: '#f5
9e0b', class: 'feed-point' },
            { x: distillateComposition, y: distillateComposition, label: 'Distil
late', color: '#ea580c', class: 'distillate-point' },
            { x: bottomsComposition, y: bottomsComposition, label: 'Bottoms', co
lor: '#059669', class: 'bottoms-point' },
            { x: operatingLines.intersection.x, y: operatingLines.intersection.y
, label: 'Intersection', color: '#7c3aed', class: 'intersection-point' }
        ];
    ...
    optimizeRefluxRatio() {
        ...
        const optimalReflux = minReflux * 1.5; // Typical optimal is 1.2-1.5 tim
es minimum
    ...
    toggleAnimation() {
        ...
        btn.textContent = this.animationEnabled ? 'Disable Animation' : 'Enable
Animation';
...
;
        ...
        if (data) {
            ...
            document.getElementById('typicalAlpha').textContent = data.alpha.toF
ixed(1);
    ...
    exportSVG() {
        ...
        const cssStyles = `
            <style>
                ...
                .axis path, .axis line { fill: none; stroke: #6b7280; stroke-wid
th: 1px; }
                .grid line { stroke: #e5e7eb; stroke-opacity: 0.7; stroke-width:
 1px; }
                .axis-label { font-family: Inter, sans-serif; font-size: 14px; f
ont-weight: 600; }
            </style>
        `;
    ...
    exportPDF() {
        ...
        this.exportPNG();
        this.showNotification('PDF export not yet implemented. PNG exported inst
ead.');
    }
    ...
    exportCSV() {
        const csvData = [
            ...
            ['Feed Composition', this.parameters.feedComposition.toFixed(3), 'mo
l/mol'],
            ['Distillate Composition', this.parameters.distillateComposition.toF
ixed(3), 'mol/mol'],
            ['Bottoms Composition', this.parameters.bottomsComposition.toFixed(3
), 'mol/mol'],
...
—'],
            ['Theoretical Plates', this.results.theoreticalPlates.toString(), 's
tages'],
            ['Feed Plate Location', this.results.feedPlateLocation.toString(), '
from top'],
