document.addEventListener('DOMContentLoaded', function() {
    ...
    const screenWidth = window.innerWidth + hexWidth;
    const screenHeight = window.innerHeight + hexHeight;
    ...
    if (typeof gsap !== 'undefined') {
        ...
        function pulseHexagon(hexElement) {
            ...
            gsap.timeline()
                .to(hexElement, {
                    strokeOpacity: Math.min(1, dynamicBaseOpacity + 0.6), // Increased opacity boost
                    strokeWidth: 2, // Increased stroke width for pulse
                    duration: 0.25, // Faster pulse up
                    ease: "power1.out"
                })
                .to(hexElement, {
                    strokeOpacity: dynamicBaseOpacity,
                    strokeWidth: 1, // Return to base stroke width
                    duration: 0.35, // Faster pulse down
                    ease: "power1.in"
                });
        }

        gsap.to({}, {
            duration: 0.07, // Faster ticker
            repeat: -1,
            ease: "linear",
            onRepeat: () => {
